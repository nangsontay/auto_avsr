...
NUM_GPUS          = 1
WORKERS_PER_GPU   = 1     # 8-core CPU: 1 worker, ~12 thread tổng (1.5x oversubscribe vừa phải)
DECODE_THREADS    = 2     # per worker — I/O wait, 2 đủ prefetch (cũ: 4 gây thrashing trên 8 core)
CROP_WORKERS      = 3     # per worker — CPU-bound, scale theo số core
SAVE_THREADS      = 2     # per worker — NVENC wait, 2 đủ (cũ: 3)
...
# Chunk size: chọn theo phân phối thực tế — median 165 frames, P95 = 450 frames.
# CHUNK=128 cho waste padding thấp nhất 10.66% (tested via clip_length_stats.py).
DET_CHUNK         = 128
FAN_CHUNK         = 128
# Chunk size cho retry pass (1 worker, full VRAM, video bị OOM trước đó)
RETRY_DET_CHUNK   = 64    # rất bảo thủ — clip bị OOM thường rất dài/HD
RETRY_FAN_CHUNK   = 64
USE_FP16          = True
...
def worker(worker_id, gpu_id, file_chunk, train_set, val_set, test_set,
    ...
    # cudnn.benchmark = False khi dùng torch.compile để tránh xung đột giữa
    # cudnn auto-tune và inductor's Triton kernel selector — cả hai cùng probe
    # heuristic ở lần forward đầu cho mỗi shape lạ → cold-start bị nhân đôi.
    # Inductor đã chọn kernel tối ưu, không cần cudnn benchmark thêm.
    torch.backends.cudnn.benchmark = not USE_COMPILE
    ...
    # ── Pre-warm torch.compile graphs ──────────────────────────────────────
    # First call per unique shape pays compile cost (~2-5s) + CUDAGraph capture
    # cost (~0.5s). Doing this upfront on dummy frames prevents the first ~5
    # real videos from suffering 10-30s extra latency. We warm both common
    # bucket sizes so production traffic always hits hot replays.
    if USE_COMPILE:
        try:
            import numpy as _np
            print(f"W{worker_id}: pre-warming torch.compile (det_chunk={det_chunk}, fan_chunk={fan_chunk})…", flush=True)
            t_warm = time.time()
            # Use the largest bucket so smaller buckets won't trigger re-compile
            # if the graph machinery happens to share kernels (it usually does).
            warm_n = max(det_chunk, fan_chunk)
            dummy = _np.zeros((warm_n, 224, 224, 3), dtype=_np.uint8)
            for _ in range(2):  # 1st = compile, 2nd = CUDAGraph replay
                fast_pipeline(dummy)
            del dummy
            torch.cuda.synchronize()
            print(f"W{worker_id}: pre-warm done in {time.time()-t_warm:.1f}s", flush=True)
        except Exception as _e:
            print(f"W{worker_id}: pre-warm failed ({_e}) — continuing anyway", flush=True)
